...
from backend.models.models import User, QuizResult, CompanyData, VideoIdea, Script, ScriptResult
...
async def submit_quiz(quiz_result: QuizResultRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Scrape company data
        company_summary = scrape_company_data(user_info.company_name, user_info.website_url)
        ...
        ideas = generate_video_ideas(influencer_name, industry, company_summary)
        ...
        for i, idea in enumerate(ideas):
            script_data = generate_script(idea, influencer_name, company_summary)
